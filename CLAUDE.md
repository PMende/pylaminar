...
This file reflects *current* rules and state only. The historical "why"
behind past decisions lives in `DECISIONS.md`, an append-only log — see
"Working With Claude" below for the rule on when to update which file.
...
The decision log — historical entries on *why* material decisions were
made — lives in `DECISIONS.md`, not here, so it can grow without making
this file harder to skim. `DECISIONS.md` is append-only: past entries are
never rewritten, only superseded by a new dated entry.

Two different files, two different jobs — when a design decision lands,
ask which (often both) apply:

- **Append to `DECISIONS.md`** when future sessions will need the *why* —
  rationale, alternatives considered, precedent — behind a decision that's
  been made or reaffirmed. This is the durable record that stops a
  question from being silently re-litigated later.
- **Amend CLAUDE.md** when the decision changes *current* prescriptive
  guidance — what Claude should actually do going forward (Architecture
  Principles, Development Conventions, Package Layout, Domain Model,
  Open Questions). CLAUDE.md should always describe the present state,
  not the path taken to reach it.
- **Resolving an Open Question** is normally both: remove the question
  from Open Questions, fold the resolution into whichever section above
  now governs it, *and* append a `DECISIONS.md` entry capturing the why.
